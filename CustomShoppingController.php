...
namespace Plugin\Introducer\Controller;
...
use Plugin\Introducer\Repository\IntroducerRepository;
use Plugin\Introducer\Repository\DiscountrateRepository;
use Plugin\Introducer\Repository\IpointRepository;

class CustomShoppingController extends AbstractShoppingController
{
    /**
     * @var CartService
     */
     ...
     */
    protected $introducerRepository;

    /**
     * @var DiscountrateRepository
     */
    protected $discountrateRepository;
    ...
    /**
     * @var IpointRepository
     */
    protected $ipointRepository;
     ...
     * @Template("@Introducer/Shopping/index.twig")
     */
    public function index(PurchaseFlow $cartPurchaseFlow)
    {
		...
		if ($this->isGranted('ROLE_USER')) {
			...
			$user = $this->getUser();
			if($user) {
		        $Introduce = $this->introducerRepository->findBy([
					'customer_id' => $user->getId(),
					'disabled' => 0
				]);
				if(!$Introduce){
			        $BeIntroduce = $this->introducerRepository->findBy([
						'intro_id' => $user->getId(),
						'disabled' => 0
					]);
					if($BeIntroduce){
			...
			if($introFlg){
		        $Rate = $this->discountrateRepository->findBy(['id' => 1]);
			}
			if($Rate){
				$Rate = ($Rate[0]->getRate() / 100);
			}
		}
		//******	追加END		******//
        ...
        $Customer = $this->getUser() ? $this->getUser() : $this->orderHelper->getNonMember();
			...
			if($introFlg){ // 割引適用
		        ...
		        foreach ($Carts as $Cart) {
		            //$totalPrice += floor($Cart->getTotalPrice() / $Rate);// 小数点切り捨て
				}
			}
		...
		// **** 追加 ****
		$Order->setTax(floor($Order->getSubtotal() - ($Order->getSubtotal() / 1.1)));
...
//dump($Order);exit;
       ...
       if ($flowResult->hasError()) {
        ...
        if ($Customer->getId()) {
			...
			// **** 追加 ****
			$Order = $this->orderDiscount($Order, $Rate);
     ...
     * @Template("@Introducer/Shopping/confirm.twig")
     */
    ...
    public function checkout(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
				...
				//******	追加	******//
				$Order = $this->orderDiscount($Order, $Rate);
				$Introducer = $this->customerRepository->findOneBy(
					[
						'intro_cd' => $Order->getCustomer()->getIntroducerCd()
					]
				);
				if($Introducer){
					$Introducer
							->setPoint($Introducer->getPoint() + $Order->getAddPoint());
				}

				$Intro_id = $Introducer->getId();
				$Ipoint_point = $Order->getAddPoint();
				$Order_id = $Order->getId();
				$this->entityManager->flush();

				if($Introducer){
					$Ipoint = $this->ipointRepository->newIpoint();
					$Ipoint
						->setPoint($Ipoint_point)
						->setCustomerId($Intro_id)
						->setOrderId($Order_id)
						->setDisabled(0);
		            $this->entityManager->persist($Ipoint);
					$this->entityManager->flush();
				}
				//******	追加END		******//
	 ...
	 *
	**/
	private function orderDiscount($Order, $Rate){
		$Order->setTax(floor($Order->getSubtotal() - ($Order->getSubtotal() / 1.1)));
		$Order->setDiscount(floor($Order->getTotalPrice() * $Rate));
		$Order->setSubTotal(floor($Order->getSubTotal() - ($Order->getSubTotal() * $Rate)));
		$Order->setTax(floor($Order->getSubtotal() - ($Order->getSubtotal() / 1.1)));
